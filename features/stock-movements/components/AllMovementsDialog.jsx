...
export function AllMovementsDialog() {
  ...
  const { data: movements, isLoading } = useQuery({
    queryKey: ['all-stock-movements', rangeKey],
    queryFn: async () => {
      const result = await getStockMovementsAction(500, rangeKey);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] sm:max-w-[95vw] h-[85vh] xl:w-[50vw] flex flex-col">
        <DialogHeader>
          <DialogTitle>Tüm Stok Hareketleri (Seçili Aralık - Son 500)</DialogTitle>
        </DialogHeader>
